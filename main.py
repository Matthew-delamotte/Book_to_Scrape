...
for category_link, value in data.items():
    ...
    for i in a_books:
        book_links.append(
            "http://books.toscrape.com/catalogue/" + found_book_link(i)
        )  # Append link of first of category

    page = 1
    while (
        response.status_code == 200
    ):  # verify if url is valid, if not restart new category page
        search = (
            category_link[:-10] + "page-" + str(page) + ".html"
        )  # Concatenate link of category and page numbers for next url
        ...
        for i in a_books:
            book_links.append(
                "http://books.toscrape.com/catalogue/" + found_book_link(i)
            )  # Found all book of page and add it to list
    ...
    ) as csv_file:  # create csv file of value name (write mode)
        fieldnames = [
            "Product_page_url",
        ...
        for link in book_links:  # scrap info of book page
            ...
            if response.ok:
                ...
                price_excl_tax = (
                    ...
                    .findNext("td")
                    .contents
                )  # return(price_excl_tax[0])
                price_excl_tax = clean_character_price(price_excl_tax)
                price_incl_tax = clean_character_price(price_incl_tax)
                # Stock scraping --------------------------------
                stock = path.xpath(
                    '//*[@id="content_inner"]/article/div[1]/div[2]/p[2]/text()'
                )
                stock = "".join(stock)
                stock = re.findall(
                    "\d+", stock
                )  # recherche toutes les décimals # write(stock[0])
                ...
                # Category scraping -----------------------------
                category = (
                    soup.find("div", {"class": "page_inner"})
                    .findNext("li")
                    .findNext("li")
                    .findNext("li")
                    .find("a")
                ...
                with open(
                    path_image
                    + clean_filename(title.text)
                    + "."
                    + image_url.split(".")[-1],
                    "wb+",
                ) as img_dl:  # Split l'url avec tout les '.', et commence la chaine par la dernier occurence.
